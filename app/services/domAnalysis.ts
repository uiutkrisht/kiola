...
export class DOMAnalysisService {
  ...
  async compareContent(figmaData: any, domAnalysis: WebsiteAnalysis): Promise<DesignIssue[]> {
    const issues: DesignIssue[] = [];

    // Convert colors to hex for better readability
    const hexColors = domAnalysis.metadata.uniqueColors.map(color => this.rgbToHex(color));
    ...
    domAnalysis.buttons.forEach(button => {
